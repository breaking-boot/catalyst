...
| `src/trainingGrounds.js` | Training Grounds difficulty filter: Easy/Medium/Hard pills injected into the challenge-search filter popover, selection state (`be_challenge_filter`), page-state pushes, and the popover-close commit that refreshes the search. The response filtering itself lives in `injected.js`. |
...
- `chrome.storage.local` keys for boss data: `be_boss_state` (tracker stats, including `eventHighAt` — when the event high was last raised), `be_boss_ui_state` (panel position/minimized), `be_boss_reminder_state` (reminder bookkeeping, per-device), `be_boss_inactive_notice` (last "no active event" toast timestamp). `be_challenge_filter` (storage.local) holds the Training Grounds difficulty selection (`{ tiers: ["easy"|"medium"|"hard", …] }`, per-device; deliberately not exported by backup.js). Feature settings live in `chrome.storage.sync` under `be_settings`. `be_import_broadcast` (storage.local) is the import signal: written by `backup.js` after a successful options-page import, consumed by `content.js` to reload imported keys in open tabs — tabs deliberately do NOT react to the data keys themselves, because they write those keys routinely and would loop on their own writes.
...
- Training Grounds difficulty filter: `injected.js` rewrites the `/v1/challenges/search` response before Vue consumes it when 1–2 tiers are selected (**fail-open**: any parse/shape problem hands Boot.dev its original response, and records with unresolvable difficulty are never hidden). The selection commits on popover close via a page-context Vue-router re-push carrying `diff=<tiers>` in the page URL (server-ignored, probe-verified; `location.reload()` fallback), and a `diff=` URL is adopted on load so shared search links carry the difficulty between Catalyst users. Tier bounds (easy 1–4 / medium 5–7 / hard 8–10) mirror Boot.dev's difficulty icons and live in two "keep in sync" copies: `DIFFICULTY_TIERS` (injected.js) and `CHALLENGE_TIERS` (trainingGrounds.js). Evidence bundle: `reference_data/catalyst_versions/v0.10.0_challenge_difficulty_filter/`.
