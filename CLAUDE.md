...
| `src/trainingGrounds.js` | Training Grounds difficulty filter: Easy/Medium/Hard pills injected into the challenge-search filter popover. Mirrors the native pills — picks are pending until Boot.dev's Search commits them; the committed state is per-tab, carried in the page URL (`diff=`), handed to `injected.js` via the `data-be-diff` attribute, and never stored. The response filtering itself lives in `injected.js`. |
...
```
utils.js → settings-schema.js → settings.js → leaderboard.js → profile.js → boss.js → nextLesson.js → updateCheck.js → trainingGrounds.js → submitConfirm.js → cliShortcuts.js → content.js
```
...
- `chrome.storage.local` keys for boss data: `be_boss_state` (tracker stats, including `eventHighAt` — when the event high was last raised), `be_boss_ui_state` (panel position/minimized), `be_boss_reminder_state` (reminder bookkeeping, per-device), `be_boss_inactive_notice` (last "no active event" toast timestamp). The Training Grounds difficulty selection is deliberately **not** stored anywhere — it is per-tab and carried in the page URL (`diff=easy,hard`), matching how the native filters derive from the URL. Feature settings live in `chrome.storage.sync` under `be_settings`. `be_import_broadcast` (storage.local) is the import signal: written by `backup.js` after a successful options-page import, consumed by `content.js` to reload imported keys in open tabs — tabs deliberately do NOT react to the data keys themselves, because they write those keys routinely and would loop on their own writes.
- Gate any new feature behind a flag in `settings-schema.js` (`SETTINGS_DEFAULTS`, default-on; the three exceptions are `versionCheck`, default-off because it can reach off-device, `bossTracker`, default-off because the panel must be quiet until the user opts in, and `submitConfirm`, default-off because it interposes a step in front of a native Boot.dev action) and a toggle rendered from `FEATURE_TOGGLES`/`PERSONAL_BOARD_TOGGLES`/`COMPARISON_BOARDS`; check `isFeatureEnabled`/`isComparisonEnabled`/`isPersonalBoardEnabled` at its render/request entry and tear down cleanly when off. Because the schema is shared, adding a key in one place covers both the content script and the settings pages.
...
- `/v1/challenges/search` — Training Grounds challenge search (`q` required, `t`/`l` optional); one response carries the complete result array (bare JSON array, no count metadata; the page paginates it locally 10/page), and `Topics.Difficulty` (1–10) drives the Catalyst difficulty filter
...
- Boss polling stops between events: a response whose `Event.ExpiresAt` is in the past marks the event inactive, halts the 2-min poll, and shows a "no active event" toast at most once per 24h per device (timestamp in `be_boss_inactive_notice`, storage.local). A response with **no readable event identity** (`Event.UUID`/`Event.StartsAt` both absent after normalization) also counts as inactive: it must never trigger the reminder toast (the missing-expiry fail-open used to make it look "active") and never resets the stored per-event stats. **Casing warning:** the between-events response was captured entirely camelCase (`event.uuid`, `xpBonus`) while live-event captures are PascalCase — `normalizeBossProgressJson` (boss.js) maps camelCase onto the PascalCase names; see `boss_events_progress_between_events.json`. If the next live event arrives camelCase, that normalizer is what keeps the tracker working. A forced re-check (navigation, manual Refresh, tab focus) or Boot.dev's own passive `boss_events_progress` fetch resumes polling when a new event starts.
...
- Lesson features (v0.11.0) are **page-side only** — no API calls, no storage keys, no observers. Both bind one `document` listener in `initEnhancer` and unbind it in `stopEnhancer`; the feature check happens per event, not per bind, so toggling a setting takes effect on the next click/keypress. The Submit button is identified by exact `Submit` text **plus** a corroborator (`#console-resizer` ancestor, or the `aria-describedby` tooltip mentioning submit) — never by hashed classes, and an unrecognized page fails open to native behavior. `Alt+R` was rejected for the run shortcut: NVIDIA's GPU-overlay global hotkey swallows the `R` keydown before the page sees it (verified), so the shortcut would have failed *silently* on those machines.
...
- Training Grounds difficulty filter: `injected.js` rewrites the `/v1/challenges/search` response before Vue consumes it when 1–2 tiers are committed (**fail-open**: any parse/shape problem hands Boot.dev its original response, and records with unresolvable difficulty are never hidden). Interaction mirrors the native pills: picks are pending until Boot.dev's **Search** commits them (a capture-phase `submit` listener writes the committed tiers into the synchronous `data-be-diff` attribute on `<html>` before the page's own handler can fetch). Boot.dev skips the refetch when q/t/l are unchanged — and **cold loads (F5/deep link) server-render the results with no API call at all** — so `trainingGrounds.js` detects both cases and asks the page to re-run the search via `BE_REFRESH_CHALLENGE_SEARCH` → a page-context Vue-router re-push with a throwaway `be_r` nonce (`location.reload()` last resort). After every search relay, `history.replaceState` strips `be_r` and keeps `diff=<tiers>` in the URL matching what actually filtered the results; a `diff=` URL is adopted on load, so shared links carry the difficulty between Catalyst users. Tier bounds (easy 1–4 / medium 5–7 / hard 8–10) mirror Boot.dev's difficulty icons and live in two "keep in sync" copies: `DIFFICULTY_TIERS` (injected.js) and `CHALLENGE_TIERS` (trainingGrounds.js). Evidence bundle: `reference_data/catalyst_versions/v0.10.0_challenge_difficulty_filter/`.
