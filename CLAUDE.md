...
- `chrome.storage.local` keys for boss data: `be_boss_state` (tracker stats, including `eventHighAt` — when the event high was last raised), `be_boss_ui_state` (panel position/minimized), `be_boss_reminder_state` (reminder bookkeeping, per-device), `be_boss_inactive_notice` (last "no active event" toast timestamp). Feature settings live in `chrome.storage.sync` under `be_settings`. `be_import_broadcast` (storage.local) is the import signal: written by `backup.js` after a successful options-page import, consumed by `content.js` to reload imported keys in open tabs — tabs deliberately do NOT react to the data keys themselves, because they write those keys routinely and would loop on their own writes.
