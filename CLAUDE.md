...
- `chrome.storage.local` keys for boss data: `be_boss_state` (tracker stats), `be_boss_ui_state` (panel position/minimized), `be_boss_reminder_state` (reminder bookkeeping, per-device). Feature settings live in `chrome.storage.sync` under `be_settings`.
- Gate any new feature behind a flag in `settings-schema.js` (`SETTINGS_DEFAULTS`, default-on; the two exceptions are `versionCheck`, default-off because it can reach off-device, and `bossTracker`, default-off because the panel must be quiet until the user opts in) and a toggle rendered from `FEATURE_TOGGLES`/`COMPARISON_BOARDS`; check `isFeatureEnabled`/`isComparisonEnabled` at its render/request entry and tear down cleanly when off. Because the schema is shared, adding a key in one place covers both the content script and the settings pages.
...
- Boss-event reminder (tracker hidden, `bossReminders` on): detection is **passive-only** — Catalyst issues zero boss requests while the tracker is off and relies on boot.dev's own relayed `boss_events_progress` responses. The toast shows at most once per 24h per event per device; either button ends reminders for that event ("Show Tracker" flips the setting via `setFeatureEnabled`, and the `storage.onChanged` live-apply does the rest). In quiet mode `handleBossProgress` never touches `be_boss_state`, so the last tracked event's stats survive until the tracker is re-enabled.
